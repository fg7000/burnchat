 ...
 * The browser UI stays completely responsive while this processes documents.
 */
...
self.onmessage = async (e: MessageEvent) => {
  ...
  if (type === "init") {
    try {
      ...
      console.log("[BurnChat Worker] GLiNER PII model loaded — running in background thread");
    } catch (err: any) {
    ...
    try {
      ...
      console.error("[BurnChat Worker] Inference failed:", err);

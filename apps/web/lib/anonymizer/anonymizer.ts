...
export async function anonymizeText(
  ...
  const contextFiltered = unique.filter((e) => {
    ...
    if (shouldKeepEntity(normalized, detectedContext)) return false;
    return true;
  });
  ...
  return { anonymizedText: result, mapping: newMapping, entitiesFound: contextFiltered.length, detectedContext };
}
